...
function TestimonialCard({
  ...
  return (
    <a
      ...
      className={cn(
        "relative flex h-auto min-h-[140px] sm:min-h-[180px] w-[260px] sm:w-[380px] -skew-y-[8deg] select-none flex-col rounded-2xl border border-white/5 bg-black backdrop-blur-sm px-3 sm:px-4 py-3 sm:py-4 transition-all duration-500 hover:border-white/10 hover:bg-gray-900 cursor-pointer",
        ...
        isActive && "ring-2 ring-draftrPurple/50",
        className
      )}
    >
      {/* Header */}
      <div className="flex items-start gap-2 sm:gap-3 mb-2 sm:mb-3">
        <div className="size-9 sm:size-12 rounded-full bg-gradient-to-br from-draftrPurple to-[#8771ff] flex items-center justify-center overflow-hidden shrink-0">
          {avatar ? (
        ...
        <TwitterIcon className="size-4 sm:size-5 text-white shrink-0" />
      </div>

      {/* Content */}
      <p className="text-white text-xs sm:text-[15px] leading-relaxed mb-2 sm:mb-3 line-clamp-3 sm:line-clamp-4">
        {content}
      </p>
...
export default function Testimonials({ cards }: TestimonialsProps) {
  ...
  const defaultCards: TestimonialCardProps[] = [
    {
      className:
        "[grid-area:stack] hover:-translate-y-10 before:absolute before:w-[100%] before:outline-1 before:rounded-2xl before:outline-white/10 before:h-[100%] before:content-[''] before:bg-blend-overlay before:bg-white/60 grayscale-[100%] hover:before:opacity-0 before:transition-opacity before:duration-500 hover:grayscale-0 before:left-0 before:top-0",
